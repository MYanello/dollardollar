...
from models import Budget, Expense, Group, Tag, User
from routes.demo_routes import create_demo_data
...
def reset_demo_data():
    ...
    try:
        ...
        for user in demo_users:
            ...
            logger.info("Deleting expenses for user: %s", user.id)
            session.query(Expense).filter(Expense.user_id == user.id).delete()

            # Delete user's budgets
            logger.info("Deleting budgets for user: %s", user.id)
            session.query(Budget).filter(Budget.user_id == user.id).delete()

            # Delete user's tags
            logger.info("Deleting tags for user: %s", user.id)
            session.query(Tag).filter(Tag.user_id == user.id).delete()

            # Find groups created by user
            logger.info("Deleting groups for user: %s", user.id)
            user_groups = (
                session.query(Group).filter(Group.created_by == user.id).all()
            )
        ...
        for user in demo_users:
            logger.info("Resetting password for user: %s", user.id)
        ...
        logger.exception("Error during demo reset")
        if session:
            session.rollback()
    finally:
        if session:
...
def main():
    ...
    logger.info("Scheduling reset every %s hours", hours)
