...
class SimpleFin:
    ...
    def decode_setup_token(self, setup_token):
        ...
        try:
            ...
            self.app.logger.exception("Error decoding token")
    ...
    def claim_access_url(self, claim_url):
        ...
        try:
            ...
            self.app.logger.error(
                f"Error claiming access URL: {response.status_code} "
                "- {response.text}"
            )
            return None
        except Exception:
            self.app.logger.exception("Error claiming access URL")
            return None
    ...
    def parse_access_url(self, access_url):
        ...
        try:
            ...
            self.app.logger.exception("Error parsing access URL")
            return None
    ...
    def get_accounts_with_transactions(self, access_url, days_back=30):
        ...
        try:
            ...
            self.app.logger.info(
                f"Fetching accounts and transactions from: {url}"
            )

            response = requests.get(
                url, auth=(parsed["username"], parsed["password"])
            )
            if response.status_code == 200:
                return response.json()
            self.app.logger.error(
                f"Error fetching accounts: {response.status_code} - {response.text}"
            )
            return None
        except Exception as e:
            self.app.logger.error(f"Error fetching accounts: {e!s}")
            return None
    ...
    def process_raw_accounts(self, raw_data):
        ...
        for account in raw_data["accounts"]:
            ...
            if "type" in account:
                account_type_lower = (
                    account["type"].lower() if account["type"] else ""
                )
                if any(
                    term in account_type_lower
                    for term in ["credit", "card", "visa", "mastercard"]
                ):
                    account_type = "credit"
                elif any(
                    term in account_type_lower for term in ["saving", "savings"]
                ):
                    ...
                    term in account_type_lower
                    for term in ["loan", "mortgage", "debt", "lend"]
                ):
                    account_type = "loan"
            ...
            processed_account = {
                ...
                "transactions": self.process_raw_transactions(
    ...
    def process_raw_transactions(self, raw_transactions):
        ...
        for trans in raw_transactions:
            ...
            if (
                "id" not in trans
                or "posted" not in trans
                or "amount" not in trans
            ):
                continue
    ...
    def test_access_url(self, access_url):
        ...
        try:
            ...
            self.app.logger.error(f"Error testing access URL: {e!s}")
            return False

    def create_transactions_from_account(
        self,
        account_data,
        db_account,
        user_id,
        detect_transfer_func=None,
        auto_categorize_func=None,
        get_category_id_func=None,
    ):
        """Create Expense model instances from processed account data.

        Apply transfer detection and auto-categorization.
        ...
        for trans in account_data.get("transactions", []):
            try:
                ...
                self.app.logger.exception("Error creating transaction")
    ...
    def create_transaction_instance(
        ...
        try:
            ...
            self.app.logger.exception(
                "Could not check for existing transaction"
            )
        ...
        if detect_transfer_func and source_account_id:
            try:
                ...
                if is_transfer:
                    ...
                    transaction_type = trans_data.get(
                        "transaction_type", "expense"
                    )
            except Exception:
            ...
            if auto_categorize_func:
                try:
                    category_id = auto_categorize_func(
                        trans_data.get("description", ""), user_id
                    )
                except Exception:
                    self.app.logger.exception("Error in auto-categorization")

            # If auto-categorization didn't find a match
            # but SimpleFin provided a category name,
