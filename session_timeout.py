...
class DemoTimeout:
    ...
    def init_app(self, app):
        ...
        app.config.setdefault(
            "MAX_CONCURRENT_DEMO_SESSIONS", self.max_concurrent_sessions
        )
    ...
    def check_session_timeout(self):
        ...
        if (
            ...
            and request.path not in ["/login", "/logout", "/demo", "/"]
            and current_user.is_authenticated
            and self.is_demo_user(current_user.id)
        ):
            # Check if session start time exists
            if "demo_start_time" not in session:
                session["demo_start_time"] = datetime.now(tz.utc).timestamp()

            # Check if session has expired
            start_time = datetime.fromtimestamp(session["demo_start_time"])
            if datetime.now(tz.utc) > start_time + timedelta(
                ...
                session.clear()
                flash(
                    "Your demo session has expired. Thank you for trying our application!"
                )
    ...
    def update_last_active(self, response):
        ...
        if current_user.is_authenticated and self.is_demo_user(current_user.id):
            session["last_active"] = datetime.now(tz.utc).timestamp()
        return response
    ...
    def is_demo_user(self, user_id):
        ...
        return (
            user_id in self.demo_users
            or user_id
            in ["demo@example.com", "demo1@example.com", "demo2@example.com"]
            or user_id.endswith("@demo.com")
            or "demo" in user_id.lower()
        )

    def get_remaining_time(self):
        """Get the remaining time for the demo session in seconds."""
        if not current_user.is_authenticated or not self.is_demo_user(
            current_user.id
        ):
            return None
        ...
        remaining = (end_time - datetime.now(tz.utc)).total_seconds()
