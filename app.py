...
app.config["SIMPLEFIN_SETUP_TOKEN_URL"] = os.getenv(
    "SIMPLEFIN_SETUP_TOKEN_URL", "https://beta-bridge.simplefin.org/setup-token"
)

app.config["GOCARDLESS_ENABLED"] = (
    os.getenv("GOCARDLESS_ENABLED", "True").lower() == "true"
)
...
def load_user(id):
    return db.select(User).filter_by(id=id).first()
...
def determine_transaction_type(row, current_account_id=None):
    ...
    amount_str = (
        row.get(amount_column, "0").strip().replace("$", "").replace(",", "")
    )
    ...
    if current_account_id:
        is_transfer, _, _ = detect_internal_transfer(
            description, amount, current_account_id
        )
        if is_transfer:
            return "transfer"
...
def update_category_mappings(transaction_id, category_id, learn=False):
    ...
    """
    transaction = db.select(Expense).get(transaction_id)
...
def create_scheduled_expenses():
    ...
    active_recurring = (
        db.select(RecurringExpense).filter_by(active=True).all()
    )
...
def calculate_iou_data(expenses, users):
    ...
    for expense in expenses:
        ...
        # If current user is in the splits (but not the payer)
            ...
            payer = db.select(User).filter_by(id=payer_id).first()
...
def utility_processor():
    ...
    def get_account_by_id(account_id):
        ...
        return db.select(Account).get(account_id)
...
def handle_comparison_request():
    ...
    if metric == "spending":
        ...
        primary_daily = process_daily_spending(
            primary_expenses, primary_start_date, primary_end_date
        )
        comparison_daily = process_daily_spending(
            comparison_expenses, comparison_start_date, comparison_end_date
        )

        # Normalize to 10 data points for consistent display
        result["primary"]["dailyAmounts"] = normalize_time_series(
            primary_daily, 10
        )
        result["comparison"]["dailyAmounts"] = normalize_time_series(
            comparison_daily, 10
        )
        ...
        all_categories = set(
            list(primary_categories.keys()) + list(comparison_categories.keys())
        )
        top_categories = sorted(
            all_categories,
            key=lambda c: (
                primary_categories.get(c, 0) + comparison_categories.get(c, 0)
            ),
            reverse=True,
        )[:5]

        result["categoryLabels"] = top_categories
        result["primary"]["categoryAmounts"] = [
            primary_categories.get(cat, 0) for cat in top_categories
        ]
        result["comparison"]["categoryAmounts"] = [
            comparison_categories.get(cat, 0) for cat in top_categories
        ]

        # Set top category
        result["primary"]["topCategory"] = (
            max(primary_categories.items(), key=lambda x: x[1])[0]
            if primary_categories
            else "None"
        )
        result["comparison"]["topCategory"] = (
            max(comparison_categories.items(), key=lambda x: x[1])[0]
        ...
        for expense in primary_expenses:
            ...
            expense_obj = db.select(Expense).get(expense["id"])
        ...
        result["primary"]["tagAmounts"] = [
            primary_tags.get(tag, 0) for tag in top_tags
        ]
        result["comparison"]["tagAmounts"] = [
            comparison_tags.get(tag, 0) for tag in top_tags
        ]
        ...
        all_methods = set(
            list(primary_payment.keys()) + list(comparison_payment.keys())
        )

        result["paymentLabels"] = list(all_methods)
        result["primary"]["paymentAmounts"] = [
            primary_payment.get(method, 0) for method in all_methods
        ]
        result["comparison"]["paymentAmounts"] = [
            comparison_payment.get(method, 0) for method in all_methods
        ]
