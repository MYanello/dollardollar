...
def setup_oidc_config(app):  # noqa: PLR0915
    ...
    app.config["LOCAL_LOGIN_DISABLE"] = (
    ...
    if oidc_enabled:
        ...
        app.config["OIDC_PROVIDER_NAME"] = os.getenv(
        ...
        if discovery_url:
            try:
                ...
                if response.status_code == OK:
                    ...
                    app.config["OIDC_AUTH_URI"] = config.get(
                        "authorization_endpoint"
                    )
                    app.config["OIDC_TOKEN_URI"] = config.get("token_endpoint")
                    app.config["OIDC_USERINFO_URI"] = config.get(
                        "userinfo_endpoint"
                    )
                    app.config["OIDC_JWKS_URI"] = config.get("jwks_uri")
                    app.config["OIDC_LOGOUT_URI"] = config.get(
        ...
        app.config["OIDC_REDIRECT_URI"] = (
            os.getenv("APP_URL", "http://localhost:5006") + "/oidc/callback"
        )
...
def register_oidc_routes(app, user, db):
    ...
    def oidc_callback():
        ...
        try:
            ...
            redirect_to = get_oidc_session(
                "redirect_to", url_for("dashboard.dashboard"), delete=True
            )
    ...
    def logout_oidc():
        ...
        has_oidc = (
            ...
            and current_user.oidc_id is not None
        )
